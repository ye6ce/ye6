...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (step === 'chat' && messages.length === 0 && selectedLesson) {
      const triggerIntro = async () => {
        setIsLoading(true);
        try {
          const text = await GeminiService.generateResponse(mode, "ابدأ بشرح الدرس مباشرة", undefined, selectedLesson.content, selectedLesson.title);
  ...
  const sendMessage = async () => {
    ...
    const userMsg: Message = { id: Date.now().toString(), role: 'user', content: input, mode, timestamp: Date.now() };
    setMessages(prev => [...prev, userMsg]);
    setInput('');
    setIsLoading(true);
    try {
      const text = await GeminiService.generateResponse(mode, input, undefined, selectedLesson?.content, selectedLesson?.title);
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <div className="glass-panel max-w-md w-full p-8 rounded-[2rem] border border-white/10 white-glow">
          <div className="flex flex-col items-center mb-8"><EduAiLogo className="w-16 h-16 text-white mb-4" /><h1 className="text-3xl font-black text-white">EduAi</h1></div>
          <form onSubmit={handleEmailLogin} className="space-y-4">
            <input type="email" placeholder="البريد الإلكتروني" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white text-right outline-none" value={email} onChange={(e) => setEmail(e.target.value)} required />
            <input type="password" placeholder="كلمة المرور" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white text-right outline-none" value={password} onChange={(e) => setPassword(e.target.value)} required />
            <button className="w-full bg-white text-black font-bold py-4 rounded-xl active:scale-95 transition-all">{authLoading ? 'جاري التحميل...' : 'دخول'}</button>
          </form>
          <button onClick={() => supabase.auth.signInWithOAuth({ provider: 'google' })} className="w-full mt-4 bg-white/5 border border-white/10 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-3">Google الدخول بواسطة</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-xl border-b border-white/5 h-20 flex items-center justify-between px-6">
        <button onClick={() => supabase.auth.signOut()} className="text-xs text-slate-500 border border-white/10 px-3 py-1 rounded-full">خروج</button>
        <div className="flex items-center gap-4 cursor-pointer" onClick={() => { setStep('specialty'); setMessages([]); }}>
          <span className="text-2xl font-black">EduAi</span><EduAiLogo className="w-10 h-10" />
        </div>
      </nav>
      ...
      <main className="pt-32 pb-40 px-6 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {SPECIALTIES.map((s) => (
              <button key={s.id} onClick={() => { setSelectedSpecialty(s.id); setStep('subject'); }} className="glass-panel p-8 rounded-[2.5rem] border border-white/10 text-right hover:border-white/30 transition-all">
                <div className="text-4xl mb-4">{s.icon}</div><h3 className="text-2xl font-bold">{s.name}</h3>
              </button>
            ))}
          </div>
        )}
            ...
            {selectedSubject.curriculum?.map((unit) => (
              <div key={unit.id} className="space-y-3">
                ...
                {unit.lessons.map((lesson) => (
                  <button key={lesson.id} onClick={() => { setSelectedLesson(lesson); setStep('mode'); }} className="glass-panel w-full p-6 rounded-2xl border border-white/5 hover:border-white/20 text-right font-bold">
                    {lesson.title}
                  </button>
                ))}
              </div>
        ...
        {step === 'mode' && (
          <div className="max-w-xl mx-auto grid gap-4">
             <button onClick={() => { setMode('fast'); setStep('chat'); }} className="glass-panel p-6 rounded-3xl border border-white/10 text-right font-bold text-xl">شرح ممتع</button>
             <button onClick={() => { setMode('quiz'); setStep('chat'); }} className="glass-panel p-6 rounded-3xl border border-white/10 text-right font-bold text-xl">تحدي الذكاء</button>
          </div>
        )}
